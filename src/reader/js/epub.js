	...
	startOffset = range.startOffset;
  	if(!range.collapsed) {
		...
		if(end.nodeType === 3) { // text node
			endElement = end; //此处本是end.parentNode,为了适应项目需求-按句标记（分字）生成准确的CFI功能,修改为end（即range.endContainer） 2018.1.21
		...
		if(endPath.split('/').length!=4 || endPath.split('/')[2]=='0' || endPath.length<=6)
		{
			return 'error';
		}
		endOffset = range.endOffset;
		// endPath = endPath.replace(startPath, '');  源码中这句话没有被注释，但导致了同一段落内范围标记的尾部CFI的一部分被''替换，导致不能复现
		if (endPath.length) 
		{
			endPath = endPath + "/";
		}
		return "epubcfi(" + base +                  "!/" + startPath + "/" + startIndex + ":" + startOffset + "," + endPath + endIndex + ":" + endOffset + ")";
	} 		//  epubcfi(    /6/4[Section0017.xhtml]  !/        4        /      4/2/1       :        0          ,      4/4/        1       :         1       )
			//                                                                                                        4/4/54/     1       :         1       )
	else 
	{
